...
echo ""
echo "=== HTTPS Setup (Recommended for Production) ==="
echo ""
echo "The server can run with HTTPS for encrypted connections."
echo "Self-signed certificates are generated automatically on first run."
echo ""
echo "To enable HTTPS in production:"
echo ""
echo "  1. Set environment variable in ecosystem.config.cjs:"
...
echo ""
echo "  3. Access via HTTPS (will show browser warning for self-signed cert):"
echo "     https://$IP_ADDR:3001"
echo ""
echo "  4. To accept the self-signed certificate in browsers:"
echo "     - Click 'Advanced' and 'Proceed to site' (Chrome)"
echo "     - Click 'Accept the Risk and Continue' (Firefox)"
echo ""
echo "Certificate files are stored in: data/certs/"
echo "  - server.crt (certificate)"
echo "  - server.key (private key)"
echo ""
echo "To regenerate certificates (e.g., if expired):"
echo "  rm -rf data/certs && pm2 restart all"
echo ""
echo "Optional: HTTP redirect (redirect HTTP to HTTPS):"
echo "  Add HTTP_REDIRECT=true to ecosystem.config.cjs"
echo "  This runs an HTTP server on port 3000 that redirects to HTTPS on 3001"
echo ""
